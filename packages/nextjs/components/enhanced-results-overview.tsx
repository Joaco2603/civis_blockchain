  ...
  useEffect(() => {
    ...
    if (filters.regions.length > 0) {
      ...
      if (regionalResults.length > 0) {
        const aggregated = generalData.map((candidate) => {
          const filtered = regionalResults.filter(
        ...
        const totalVotes = aggregated.reduce(
          (sum, item) => sum + item.votes,
          0
        );
    ...
    if (filters.demographics.length > 0) {
      ...
      filters.demographics.forEach((demo) => {
        if (demographicData[demo as keyof typeof demographicData]) {
          demographicResults = [
            ...demographicResults,
            ...demographicData[demo as keyof typeof demographicData]
          ];
        }
      });
      ...
      if (demographicResults.length > 0) {
        const aggregated = generalData.map((candidate) => {
          const filtered = demographicResults.filter(
            (item) => item.name === candidate.name
          );
          const totalVotes = filtered.reduce(
            (sum, item) => sum + item.votes,
    ...
    if (filters.ageRange[0] !== 18 || filters.ageRange[1] !== 80) {
      ...
      if (ageResults.length > 0) {
        const aggregated = generalData.map((candidate) => {
          const filtered = ageResults.filter(
            (item) => item.name === candidate.name
          );
          const totalVotes = filtered.reduce(
            (sum, item) => sum + item.votes,
                        ...
                        if (active && payload && payload.length) {
                          ...
                          return (
                            <div className="bg-white dark:bg-gray-800 p-2 rounded-md border shadow-sm">
                              ...
                              <p className="text-sm">
                                {data.percentage}% ({formatNumber(data.votes)}{" "}
                                votos)
                              </p>
                            </div>
                          );
                  ...
                  >
                    ...
                    <Tooltip
                      content={({ active, payload, label }) => {
                        if (active && payload && payload.length) {
                          const candidate = filteredData.find(
                            (c) => c.name === label
                          );
                          return (
                            <div className="bg-white dark:bg-gray-800 p-2 rounded-md border shadow-sm">
                              ...
                              <p className="text-sm">
                                {payload[0].value}% (
                                {formatNumber(candidate?.votes || 0)} votos)
                              </p>
                            </div>
                          );
                        }
                  ...
                  >
                    <XAxis
                      ...
                      domain={[
                        0,
                        Math.ceil(
                          Math.max(...filteredData.map((d) => d.percentage)) *
                            1.1
                        )
                      ]}
                      tickFormatter={(value) => `${value}%`}
                        ...
                        if (active && payload && payload.length) {
                          ...
                          return (
                            <div className="bg-white dark:bg-gray-800 p-2 rounded-md border shadow-sm">
                              ...
                              <p className="text-sm">
                                {payload[0].value}% (
                                {formatNumber(data?.votes || 0)} votos)
                              </p>
                            </div>
                          );
                        }
